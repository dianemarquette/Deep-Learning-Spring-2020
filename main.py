...
def project_2():
	...
	# Training
	train_model(net, train_input, train_labels, MINI_BATCH,EPOCK)
	print(net)
	# Testing
	print('train_error {:.02f}% test_error {:.02f}%'.format(
        compute_nb_errors(net, train_input, train_labels,MINI_BATCH) / train_input.size(0) * 100,
        compute_nb_errors(net, test_input, test_labels,MINI_BATCH) / test_input.size(0) * 100
        )
        )
...
def train_model(network, train_input, train_target, mini_batch_size, nb_epoch):
	...
	for e in range(nb_epoch):
		...
		for b in range(0, train_input.size(0), mini_batch_size):
			...
			if ( train_target[b,:].max(0)[1] != output.max(0)[1]):
				nb_errors = nb_errors + 1

		print(e,sum_loss)
		print("errors = ",nb_errors,"/",train_input.shape[0])
		nb_errors =0
